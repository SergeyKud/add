...
Функция СоздатьОтчетНаСервере(Знач РезультатыТестирования, Знач ТипЗапуска = "-Обычное приложение") Экспорт

	МассивОписанийШагов = СоздатьОтчетАллюр2(РезультатыТестирования, ТипЗапуска);
...
Функция ПолучитьРезультатТестаНаСервере(Знач РезультатТеста, Знач ТипЗапуска = "-Обычное приложение") Экспорт

	ОписаниеШага = СоздатьОписаниеШага(РезультатТеста, Неопределено, ТипЗапуска);
...
Функция СоздатьОтчетАллюр2(РезультатыТестирования, ТипЗапуска)
	...
	Для каждого РезультатШага Из МассивШагов Цикл
		ОписаниеШага = СоздатьОписаниеШага(РезультатШага, КоллекцияРодителей, ТипЗапуска);
...
Функция СоздатьОписаниеШага(Знач РезультатШага, Знач КоллекцияРодителей = Неопределено, ТипЗапуска)
	...
	ОписаниеШага.Вставить("name", ТипЗапуска + РезультатШага.Представление);
	...
	ОписаниеШага.historyId = СтрШаблон_("%1.%2.%3.%4", ИмяФичи, ИмяСтори, РезультатШага.Представление, ТипЗапуска);
